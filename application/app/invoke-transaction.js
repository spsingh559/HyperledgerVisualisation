...
var invokeChaincode = function(peerNames, channelName, chaincodeName, fcn, args, username, org,socket) {
	...
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		socket.emit('sendTxProposal', 'Tx Proposal Request Started');
		return channel.sendTransactionProposal(request);
	}, (err) => {
		socket.emit('sendTxProposalDecline', 'Tx Proposal Decline');
		logger.error('Failed to enroll user \'' + username + '\'. ' + err);
		throw new Error('Failed to enroll user \'' + username + '\'. ' + err);
	}).then((results) => {
		socket.emit('proposalResponse', 'proposal Response sent');
		...
		if (all_good) {
			socket.emit('proposalReceived','Successfully sent Proposal and received ProposalResponse' );
			logger.debug(util.format(
		...
		if (response.status === 'SUCCESS') {
			socket.emit('finalStatus','Successfully sent transaction to the orderer' );
